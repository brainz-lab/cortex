...
# Cortex - AI/ML Operations Platform
...
BrainzLab.configure do |config|
  ...
  config.service = "cortex"
  config.environment = Rails.env

  # Ignore internal BrainzLab hosts to prevent infinite recursion
  config.http_ignore_hosts = %w[localhost 127.0.0.1 recall reflex pulse signal flux cortex]
end
...
Rails.application.config.after_initialize do
  ...
  Rails.logger.info "[Cortex] SDK initialized"
  Rails.logger.info "[Cortex] Recall logging: #{BrainzLab.configuration.recall_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Cortex] Reflex error tracking: #{BrainzLab.configuration.reflex_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Cortex] Pulse APM: #{BrainzLab.configuration.pulse_enabled ? 'enabled' : 'disabled'}"
end
